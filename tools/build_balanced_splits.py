...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create balanced validation/test splits by pairing each eligible real image with one random AI image for the same caption.",
    )
    parser.add_argument(
        "--export-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Splits to rebalance.",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=42,
        help="Random seed used to select one AI example per prompt.",
    )
...
def main() -> None:
    ...
    for split in args.splits:
        balanced_rows = build_balanced_split(args.export_root, split, args.seed)
